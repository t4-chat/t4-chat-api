...
class AiModelService:
    ...
    @convert_to_dto
    async def get_ai_models(self, only_active: bool = True, custom_filter: Optional[ColumnElement] = None) -> List[
        AiProviderModelDTO]:
        filters = []

        if only_active:
            filters.append(AiProviderModel.is_active == True)
        ...
        models = await self.ai_model_repo.select(filter=final_filter,
                                                 includes=[AiProviderModel.provider, AiProviderModel.host_associations,
                                                           AiProviderModel.hosts])
    ...
    async def get_model_by_path(self, path: str) -> Optional[AiProviderModelDTO]:
        ...
        model = await self.ai_model_repo.get(
            ...
            filter=and_(AiProvider.slug == provider_slug, AiProviderModel.name == model_name),
    ...
    async def add(self, ai_model_dto: EditAiModelDTO) -> AiProviderModelDTO:
        ...
        ai_model = AiProviderModel(
            ...
            modalities=ai_model_dto.modalities,
    ...
    async def update(self, id: UUID, ai_model_dto: EditAiModelDTO) -> AiProviderModelDTO:
        existing_model = await self.ai_model_repo.get(
            filter=AiProviderModel.id == id,
            includes=[AiProviderModel.provider, AiProviderModel.host_associations, AiProviderModel.hosts]
        )
        ...
        existing_model.modalities = ai_model_dto.modalities
        existing_model.host_associations.clear()
        existing_model.host_associations = [
            ModelHostAssociation(host_id=assoc.host_id, model_slug=assoc.model_slug, priority=assoc.priority) for assoc
            in ai_model_dto.host_associations
        ]
    ...
    async def delete(self, id: UUID) -> None:
        ...
        if len(dependent_records) > 0:
            raise BadRequestError(
                f"Cannot delete AI model. It is still referenced by: {', '.join(dependent_records)}. " f"Please remove or reassign these records before deleting the model."
            )
