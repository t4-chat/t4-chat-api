...
class ChatApiUser(BaseUser):
    ...
    def create_new_chat(self) -> Optional[str]:
        with self.client.post(
            "/api/chats", name="Create Chat", catch_response=True
        ) as response:
                    ...
                    for event in client.events():
                        if "error" in event.data.lower():
                            ...
                            with self.client.request(
                                "GET",
                                "/placeholder",
                                name="SSE Message Error",
                                catch_response=True,
                            ) as caught:
                                caught.failure(
                                    "Error in SSE message: " + event.data[:100]
                                )
                            return
